...
def _bloc_identite(pdf: PDFRestitution, client: Client):
    y0 = pdf.get_y()
    h = 20
    ...
    pdf.cell(0, 7, _txt(f"Etude preparee pour {nom_complet}"), ln=True)
...
def _tableau_comparatif(pdf: PDFRestitution, offres: list[dict], offre_recommandee_id: int | None):
    ...
    lignes = [
        ...
        ("Prix mensuel", lambda o: f"{o.get('prix_mensuel', 0) or 0:.2f} EUR"),
        ("Economie / mois", lambda o: f"{o.get('economie_mensuelle', 0) or 0:.2f} EUR"),
        ("Economie / an", lambda o: f"{(o.get('economie_mensuelle', 0) or 0) * 12:.2f} EUR"),
    ]
    hauteur_ligne = 8
    hauteur_entete = 10
    ...
    pdf.multi_cell(0, 4, _txt(
        "Lignes du tableau, de haut en bas : Fournisseur, Offre, Prix mensuel, Economie mensuelle, Economie annuelle."))
    pdf.set_text_color(0, 0, 0)
    pdf.ln(2)
